// Practice_15_9_4.cpp : 定义控制台应用程序的入口点。
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	{
		//runQueries(input);
		Query q = Query(string("Alice")) | Query(string("hair"));
		TextQuery  TQ(input);
...
{
	shared_ptr<set<size_t>> sp = result.lines;
	shared_ptr<vector<string>> vec_sp = result.file;
	try
	{
